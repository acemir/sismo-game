define(['Phaser','MyGame'], function(Phaser, MyGame) {
	...
	MyGame.GameTitle.prototype = {
	     create: function(){
	          ...
	          var playButton = this.add.button(this.game.width / 2 , this.game.height / 2 + 80, playData, function(){this.state.start('GameDesvio')}.bind(this));
	          playButton.anchor.set(0.5);
	          var playDesvio = this.game.add.text(0, 0, 'Modo Desvio', { font: '30px Arial', fill: '#020028', align: 'center', boundsAlignH: 'center', boundsAlignV: 'middle', wordWrap: true, wordWrapWidth: playData.width });
	          playDesvio.anchor.set(0.5);
	          playButton.addChild(playDesvio);

	          var playButton = this.add.button(this.game.width / 2 , this.game.height / 2 - 80, playData, function(){this.state.start('GameRastro')}.bind(this));
	          playButton.anchor.set(0.5);
	          var playTrail = this.game.add.text(0, 0, 'Modo Rastro', { font: '30px Arial', fill: '#020028', align: 'center', boundsAlignH: 'center', boundsAlignV: 'middle', wordWrap: true, wordWrapWidth: playData.width });
	          playTrail.anchor.set(0.5);
	          playButton.addChild(playTrail);

	          var author = this.add.text(this.game.width / 2, this.game.height - 40, 'Acemir Sousa Mendes', { font:'20px Arial', fill: '#acacac', align: 'center', boundsAlignH: 'center', boundsAlignV: 'middle'});
